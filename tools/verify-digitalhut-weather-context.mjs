...
assert.match(drawerSource, /supabase\.auth\.signInWithOtp\(\{email:memberEmail\.trim\(\)\}\)/)
assert.match(drawerSource, /type="email" autoComplete="email"/)
assert.match(drawerSource, /"Sign up"/)
assert.match(drawerSource, /Owner \/ Operator Login/)
...
console.log(JSON.stringify({ok:true, checks:36, units:{temperature:"fahrenheit", precipitation:"inch"}, approximateLocationOnly:true, nonOverlayPlacement:true, marketTransportSeparated:true, memberSignupMounted:true, mixpostAdminProtected:true, cancelledDragPreservesNextClick:true, flashFloodClaimed:false}, null, 2))
