...
async function resolveApiFeeds(category, term){
  ...
  const endpoints = [
    // Sketchfab performs several provider searches and can legitimately take
    // longer than the generic feed timeout. Let the live 3D layer finish.
    [`sketchfab`, `/api/sketchfab?category=${encodedCategory}&query=${query}`, 9000],
    [`observatory`, `/api/observatory?category=${encodedCategory}&query=${query}`, 3000],
    [`observatory-feed`, `/api/observatory-feed?category=${encodedCategory}&query=${query}`, 3000]
  ]

  const attempts = await Promise.allSettled(endpoints.map(async ([source, endpoint, timeoutMs]) => {
...
export default function FullscreenObservatoryV2(){
  ...
  useEffect(() => {
    ...
    const loadPaypalButtons = (customId) => {
      ...
      if(existing) {
        // A failed SDK element never fires "load" again. Remove it so Retry
        // performs a real PayPal SDK reload instead of waiting forever.
        existing.remove()
      }
 ...
 …4 tokens truncated…y(item.query || item.title)
  ...
  return <main className={observatoryClassName} data-main-frame={digitalHutBrainMap.mainFrame} data-observatory-category={category} data-observatory-status={loading ? "verifying" : sceneFeed.apiStatus || "ready"} data-physical-assets="sensitive" data-performance-profile={performanceProfile.id} data-performance-reason={performanceProfile.reason} onPointerMove={handleObservatoryPointer} onPointerLeave={centerCockpitMotion} onPointerDown={(event) => {wake(); triggerSystemPulse(event)}} onClickCapture={triggerSystemPulse}>
    <section className="dh-stage">
      ...
      {accountSession?.user && <button
        type="button"
        className="dh-static-profile"
        onClick={() => {setAccountView("profile"); setEntryOpen(true)}}
        aria-label="Open main profile page"
      >
        <span>Profile</span>
        <b>{username || accountSession.user.email}</b>
        <small>{tier} · {visibleStructuredNodes.length} system nodes</small>
      </button>}
