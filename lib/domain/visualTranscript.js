...
export function buildVisualTranscript(feed = {}) {
  ...
  const terrainUrl = feed.terrainUrl || feed.query || "none selected"
  const visualDetail =
...
export function enrichActiveFeed(feed = {}, options = {}) {
  const feedWithVisual = enrichFeedWithResolvedVisual(feed, options)
