...
function buildSketchfabEmbedUrl(uid) {
  return uid ? `https://sketchfab.com/models/${uid}/embed?autostart=0&ui_infos=0&ui_controls=1&ui_watermark=0` : null
}
...
export async function testSketchfabModelSource(query) {
  ...
  try {
    ...
    const sampleImage = model.thumbnails?.images?.[0]?.url || null
    return {
      provider: "sketchfab",
      category: "3d-model",
      visualType: "sketchfab-embed",
      keyPresent: Boolean(token),
      ...
      sampleEmbedUrl: buildSketchfabEmbedUrl(model.uid),
      samplePageUrl: model.viewerUrl || model.url,
      author: model.user?.displayName || model.user?.username || null,
      canRender: Boolean(model.uid || download.url || sampleImage),
    ...
    return {
      provider: "sketchfab",
      category: "3d-model",
      visualType: "model-request-error",
      keyPresent: Boolean(token),
